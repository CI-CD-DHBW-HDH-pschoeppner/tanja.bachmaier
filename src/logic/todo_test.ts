...
describe('generateColor', () => {
  ...
  it('returns a string with values between 50 and 150', () => {
    ...
    expect(match?.every(val => parseInt(val) >= 50 && parseInt(val) <= 150)).toBe(true);
  });
});
...
describe('todoList', () => {
  ...
  it('can add new todo items', () => {
    ...
    store.update((todos: any) => [...todos, { id: '1', value: 'test', done: false }]);
    expect(todoList).toEqual([{ id: '1', value: 'test', done: false }]);
  });

  it('can update the done property of a todo item', () => {
    const store = todoList;
    store.update(todos => todos.map(todo => todo.id === '1' ? { ...todo, done: true } : todo));
